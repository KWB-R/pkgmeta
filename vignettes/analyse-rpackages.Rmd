...
is_travis <- if (identical(Sys.getenv("TRAVIS"), "true")) {
        TRUE } else { FALSE }
```
...
pkgs <- pkgmeta::get_github_packages(ignore_pkgs = "kwb.read")
...
```{r, echo = !is_travis, eval = !is_travis}
...
pkgmeta::write_pkg_codemeta(codemeta)
                                                     
```


```{r, echo=FALSE, eval = is_travis}

all_pkgs_installed <- all(pkgs$name %in% installed.packages()[,"Package"])

is_travis_and_all_pkgs_installed <-  is_travis && all_pkgs_installed

```



```{r, echo = is_travis, eval = is_travis_and_all_pkgs_installed}
